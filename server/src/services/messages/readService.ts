...
/**
 * List the message history of a single conversation, oldest first (chat order).
 *
 * Ordered by `ts` asc so the client can append straight to the bottom of the thread.
 * Scoped to the conversation via a `conversationId ==` filter on the tenant's flat
 * messages collection.
 */
export async function listMessages(
  ...
  // Oldest-first (chat order); id tie-breaks the (non-unique) ts ordering for stable paging.
  ...
  const items: MessageDTO[] = page.map(toMessage);
